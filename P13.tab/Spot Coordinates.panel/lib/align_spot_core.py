...
def is_spot_coordinate(element):
    ...
    try:
        ...
        return category_id == int(DB.BuiltInCategory.OST_SpotCoordinates)
    except Exception:
        return False


def belongs_to_view(element, active_view):
    try:
        return element.OwnerViewId.Value == active_view.Id.Value
    except Exception:
        return element.OwnerViewId == active_view.Id
...
def get_selected_spots(uidoc, doc, active_view):
    ...
    for element_id in uidoc.Selection.GetElementIds():
        ...
        if is_spot_coordinate(element) and belongs_to_view(element, active_view):
...
mode = globals().get("MODE_OVERRIDE")
if not mode:
