...
class ScheduleOption(forms.TemplateListItem):
    ...
    def name(self):
        try:
            itemized = self.item.Definition.IsItemized
        except Exception:
            itemized = False
        return "{}{}".format(self.item.Name, "" if itemized else " [Grouped - export only]")


class ScheduleDataBuilder(object):
    @staticmethod
    ...
    def build_sheet_data(schedule, used_sheet_names):
        ...
        return {
            ...
            "writable_count": stats["writable_count"],
...
class ExportManager(object):
    ...
    def export_separate_files(self, selected_schedules, csv_only=False):
        ...
        for schedule in selected_schedules:
            ...
            try:
                sx.write_xlsx(path, [{"name": sheet_data["name"], "rows": sheet_data["rows"]}])
            except Exception:
                path = self.unique_file_path(base_name, ".csv")
                sx.write_csv(path, sheet_data["rows"])
            created_files.append(path)
...
def main():
    ...
    if not schedules:
        forms.alert("No valid schedules were found in the current model.", exitscript=True)

    selected = forms.SelectFromList.show(
        [ScheduleOption(schedule) for schedule in schedules],
    ...
    mode = forms.CommandSwitchWindow.show(
        ["Single XLSX Workbook", "Separate XLSX Files", "Separate CSV Files"],
        message="Choose export mode"
    )
    if not mode:
        script.exit()
    ...
    if mode == "Single XLSX Workbook":
        ...
        created = exporter.export_separate_files(selected, csv_only=(mode == "Separate CSV Files"))
