...
group_vary_status = allow_vary_between_groups(
    doc,
    ["Base_Level", "Bottom of Door", "Top of Door"]
)
...
with forms.ProgressBar(title='กำลังคำนวณพารามิเตอร์ประตู... ({value} จาก {max_value})', cancellable=True) as pb:
    for index, door in enumerate(doors):
        ...
        try:
            is_in_group = is_element_in_group(door)
            ...
            if is_in_group:
                ...
                error_log.append(
                    "Door ID {}: skipped because it is inside a model group.".format(
                        get_element_id_value(door.Id)
                    )
                )
                continue
...
if skipped_group_count:
    output.print_md("### Grouped Doors")
    output.print_md(
        "Skipped **{}** grouped doors to prevent Revit from forcing the Ungroup workflow.".format(
            skipped_group_count
        )
    )
