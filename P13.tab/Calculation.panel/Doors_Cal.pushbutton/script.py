# -*- coding: utf-8 -*-
"""Update Window Parameters: Top of Doors & Bottom of Doors for Revit 2026"""
...
# -*- coding: utf-8 -*-
"""Update Door Parameters: Top of Door & Bottom of Door for Revit 2026"""
...
# =====================================================
output.print_md("### **ตรวจสอบและเตรียม Parameters (ประตู)**")
cat_doors = ["OST_Doors"]

status_base   = setup_parameter(doc, app, "Base_Level",        "Text",   cat_doors)
status_bottom = setup_parameter(doc, app, "Bottom of Door", "Length", cat_doors)
status_top    = setup_parameter(doc, app, "Top of Door",    "Length", cat_doors)
...
if status_bottom == "created": output.print_md("✅ **Bottom of Door** ถูกสร้างอัตโนมัติ")
elif status_bottom in ["exists", "updated"]: output.print_md("✅ พบพารามิเตอร์ **Bottom of Door** พร้อมใช้งาน")

if status_top == "created":    output.print_md("✅ **Top of Door** ถูกสร้างอัตโนมัติ")
elif status_top in ["exists", "updated"]: output.print_md("✅ พบพารามิเตอร์ **Top of Door** พร้อมใช้งาน")
...
if not doors:
    forms.alert("ไม่พบประตูในโมเดล", exitscript=True)

output.print_md("### **ค้นพบประตูทั้งหมด: {} รายการ**".format(len(doors)))


# =====================================================
# เริ่ม Transaction เขียนค่าลงโมเดล
# =====================================================
t = DB.Transaction(doc, "Set Door Top & Bottom Parameters")
t.Start()
...
with forms.ProgressBar(title='กำลังคำนวณพารามิเตอร์ประตู... ({value} จาก {max_value})', cancellable=True) as pb:
    for index, door in enumerate(doors):
        ...
        try:
            ...
            # ประตูใช้ FAMILY_LEVEL_PARAM (level ที่ติดตั้ง)
            lvl_param = door.get_Parameter(DB.BuiltInParameter.FAMILY_LEVEL_PARAM)
            if not lvl_param:
                error_log.append("Door ID {}: ไม่พบ Level Parameter".format(door.Id.Value))
                continue

            lvl_id = lvl_param.AsElementId()
            if lvl_id == DB.ElementId.InvalidElementId:
                error_log.append("Door ID {}: Level ID ไม่ถูกต้อง".format(door.Id.Value))
                continue
            ...
            # --- 2. ดึง Sill Height ---
            # INSTANCE_SILL_HEIGHT_PARAM = ระยะจาก Level ถึงขอบล่างประตู (Instance)
            sill_param = door.get_Parameter(DB.BuiltInParameter.INSTANCE_SILL_HEIGHT_PARAM)
            if not sill_param or not sill_param.HasValue:
                sill_param = door.LookupParameter("Sill Height")

            # Fallback: ดึง Sill Height จาก Element Type
            if not sill_param or not sill_param.HasValue:
                door_type_for_sill = doc.GetElement(door.GetTypeId())
                if door_type_for_sill:
                    sill_param = door_type_for_sill.LookupParameter("Default Sill Height")
                    if not sill_param or not sill_param.HasValue:
                        sill_param = door_type_for_sill.LookupParameter("Sill Height")

            sill_height = sill_param.AsDouble() if (sill_param and sill_param.HasValue) else 0.0

            # --- 3. ดึง Door Height ---
            # ดึง Door Height: Instance ก่อน → fallback ไป Type
            height_param = door.get_Parameter(DB.BuiltInParameter.FAMILY_HEIGHT_PARAM)
            if not height_param or not height_param.HasValue:
                height_param = door.LookupParameter("Height")

            # Fallback: ดึงจาก Element Type (Symbol) เมื่อ Height เป็น Type Parameter
            if not height_param or not height_param.HasValue:
                door_type = doc.GetElement(door.GetTypeId())
                if door_type:
                    height_param = door_type.get_Parameter(DB.BuiltInParameter.FAMILY_HEIGHT_PARAM)
                    if not height_param or not height_param.HasValue:
                        height_param = door_type.LookupParameter("Height")

            # Fallback สุดท้าย: สแกนหา Length Parameter ที่มีคำว่า "height" / "ht" ใน Type
            if not height_param or not height_param.HasValue:
                door_type2 = doc.GetElement(door.GetTypeId())
                candidates = []
                if door_type2:
                    for p in door_type2.Parameters:
            ...
            if not height_param or not height_param.HasValue:
                ...
                door_type3 = doc.GetElement(door.GetTypeId())
                diag_names = []
                if door_type3:
                    for p in door_type3.Parameters:
                        if p.StorageType == DB.StorageType.Double and p.HasValue:
            ...
            # --- 4. คำนวณสูตร ---
            # Bottom of Door = Base_Level + Sill Height
            bottom_val = base_elev + sill_height
            # Top of Door   = Base_Level + Sill Height + Height
            top_val    = base_elev + sill_height + win_height

            # --- 5. เขียนค่าลง Parameter ---

            # (A) Base_Level (Text)
            p_base = door.LookupParameter("Base_Level")
            if p_base and not p_base.IsReadOnly:
                is_in_group = hasattr(door, 'GroupId') and door.GroupId != DB.ElementId.InvalidElementId
            ...
            # (B) Bottom of Door
            p_bottom = door.LookupParameter("Bottom of Door")
            if p_bottom and not p_bottom.IsReadOnly:
                p_bottom.Set(bottom_val)

            # (C) Top of Door (เป้าหมายหลัก)
            p_top = door.LookupParameter("Top of Door")
            if p_top and not p_top.IsReadOnly:
                p_top.Set(top_val)
                success_count += 1
            elif not p_top:
                error_log.append("Door ID {}: ไม่พบ Parameter 'Top of Door'".format(door.Id.Value))

        except Exception as e:
            error_log.append("Door ID {}: {}".format(door.Id.Value, str(e)))
...
output.print_md("\n**เสร็จสิ้น — อัปเดตข้อมูลพารามิเตอร์ประตูเรียบร้อย**")
...
status_bottom = setup_parameter(doc, app, "Bottom of Doors", "Length", cat_windows)
status_top    = setup_parameter(doc, app, "Top of Doors",    "Length", cat_windows)

if status_base == "created":   output.print_md("✅ **Base_Level** (Text) ถูกสร้างอัตโนมัติ")
elif status_base in ["exists", "updated"]: output.print_md("✅ พบพารามิเตอร์ **Base_Level** พร้อมใช้งาน")

if status_bottom == "created": output.print_md("✅ **Bottom of Doors** ถูกสร้างอัตโนมัติ")
elif status_bottom in ["exists", "updated"]: output.print_md("✅ พบพารามิเตอร์ **Bottom of Doors** พร้อมใช้งาน")

if status_top == "created":    output.print_md("✅ **Top of Doors** ถูกสร้างอัตโนมัติ")
elif status_top in ["exists", "updated"]: output.print_md("✅ พบพารามิเตอร์ **Top of Doors** พร้อมใช้งาน")
...
with forms.ProgressBar(title='กำลังคำนวณพารามิเตอร์หน้าต่าง... ({value} จาก {max_value})', cancellable=True) as pb:
    for index, win in enumerate(windows):
        ...
        try:
            ...
            # --- 4. คำนวณสูตร ---
            # Bottom of Doors = Base_Level + Sill Height
            bottom_val = base_elev + sill_height
            # Top of Doors   = Base_Level + Sill Height + Height
            top_val    = base_elev + sill_height + win_height
            ...
            # (B) Bottom of Doors
            p_bottom = win.LookupParameter("Bottom of Doors")
            if p_bottom and not p_bottom.IsReadOnly:
                p_bottom.Set(bottom_val)

            # (C) Top of Doors (เป้าหมายหลัก)
            p_top = win.LookupParameter("Top of Doors")
            if p_top and not p_top.IsReadOnly:
                p_top.Set(top_val)
                success_count += 1
            elif not p_top:
                error_log.append("Window ID {}: ไม่พบ Parameter 'Top of Doors'".format(win.Id.Value))
